...
def directed_montecarlo(molecule):
    for it in range(n_mc): #Montecarlo simulation 
        atoms_ene = ene_lj_atoms(molecule) #calculate the energy for each atom
        ...
        ene_active = atoms_ene[o] #get the energy for the atoms ene
        cluster = np.delete(molecule, [3*o,3*o+1,3*o+2]) #cluster with no o atom
        atoms_ene_cluster = ene_lj_atoms(cluster)
        weight_target = np.exp(atoms_ene_cluster/Etarget)
        probability_target = weight_target/np.sum(weight_target)
        q = np.random.choice(index_cluster,p = probability_target)
        vert = grid(molecule, q)
        vertEnergies = np.zeros(8)
        for numVert in range(8):
            xn = vert[3*numVert]
            yn = vert[3*numVert+1]
            zn = vert[3*numVert+2]
            vertEnergies[numVert] = ene_lj_atom(molecule,xn,yn,zn,o)
        ...
        if dene < 0:
            ...
            if shi < prob: #update the cluster if shi is smaller than the probability
                ...
                molecule[3*o+1] = yn
                molecule[3*o+2] = zn
    energy = ene_lj(molecule)
    return molecule,energy

def operator_unicollision(m1):
    m1,ene = directed_montecarlo(m1)
    return m1,ene
...
def operator_collision(m1,m2):
    ...
    m1,ene1 = directed_montecarlo(m1)

    m2,ene2 = directed_montecarlo(m2)
...
for EaDMC in [0.6]:
    ...
    KineticEnergyLossRate = 0.8 #kinetic energy reduction
    MoleColl = 0.58 #intermolecular probability
    buffer = 1.0E04 #surroundings energy
    atoms = 38 #number of atoms for the cluster
    trigger = 3
    Eactive = 1.5 #parameter for directed montecarlo
    Etarget = 1.5 #parameter for directed montecarlo
    index = np.arange(atoms) #indexes for the directed montecarlo
    index_cluster = np.arange(atoms-1)
    ...
    for cro in range(N):
        ...
        if b > MoleColl: #80% unimolecular, #20% intermolecular
            molWeights = np.exp(PotentialEnergy/5.0)
            molProb = molWeights/np.sum(molWeights)
            molIndex = np.arange(PopSize)
            i = np.random.choice(molIndex,p = molProb)
            mol = w[i,:] #select a random molecule
            if NumHit[i] > trigger: #decomposition trigger
                deco = deco + 1
                #--------------------------------Decomposition----------------------------------------
            ...
            if PopSize > 1: #its intermolecular only if the PopSize is more than one
                ...
                m1 = w[i,:]
                m2 = w[j,:]
                if NumHit[i] and NumHit[j] > trigger:
                    syn = syn + 1
                    #-----------------Synthesis---------------------------------
        ...
        if round(comparator,4) == ReportedValue:
            IterationsNeeded = cro + 1
            break
        else:
            IterationsNeeded = cro + 1

    ActualMinStruct = np.resize(ActualMinStruct,[atoms,3])
    pDeco = deco*100/IterationsNeeded
    pUnicol = unicol*100/IterationsNeeded
    pSyn = syn*100/IterationsNeeded
    pIntcol = intcol*100/IterationsNeeded
    et = time.time()
    res = et - st #print the time used for optimization
    resmin = res/60 #change to minutes
    reshour = resmin/60
    #Create txt files for backups
    filestruct = open(f"lj{atoms}_xyz.txt","w")
    filestruct.write(str(ActualMinPotentialEnergy-offset) + "\n")
    filestruct.write(str(ActualMinStruct) + "\n")
    filestruct.close()
    fileinfo = open(f"info{atoms}.txt","w")
    ...
    fileinfo.write(f"Trigger = {trigger}\n")
    fileinfo.write(f"EaDMC = {round(EaDMC,2)}\n")
    fileinfo.write(f"IterationsNeeded = {IterationsNeeded}")
    fileinfo.close()
