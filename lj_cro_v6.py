...
def operator_unicollision(m1):
    ...
    for it in range(n_mc): #Montecarlo simulation for molecule 1
        ...
        if o != q:
            ...
            sigma = np.random.randint(8) #generate a sigama value for the 
            xn = vert[3*sigma]
            yn = vert[3*sigma+1]
            zn = vert[3*sigma+2]
...
print(f'ActualMinPotentialEnergy = {ActualMinPotentialEnergy}\n')
...
for cro in range(N):
    ...
    if MinPotentialEnergy.min() < ActualMinPotentialEnergy:
    ...
    print(cro,ActualMinPotentialEnergy,PopSize,buffer)
